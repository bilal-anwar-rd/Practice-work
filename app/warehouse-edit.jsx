...
const WarehouseEditScreen = () => {
  const router = useRouter();
  const [name, setName] = useState("Main Store");
  const [type, setType] = useState("Distribution Center");
  const [address, setAddress] = useState("123 Industrial Way, North Wing");
  const [isDefault, setIsDefault] = useState(true);
  const [isActive, setIsActive] = useState(true);
  const [images, setImages] = useState([IMAGE_LIBRARY[0], IMAGE_LIBRARY[1]]);
  ...
  return (
    <View style={styles.root}>
      <Appbar.Header mode="small" style={styles.appbar}>
        ...
        <Appbar.Content title="Edit Warehouse" titleStyle={styles.title} />
